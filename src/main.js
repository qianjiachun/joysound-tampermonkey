...
const LOCAL_JS = "Ex_joysoundJs"; // 本地wasm代码
const LOCAL_JS_VERSION = "Ex_joysoundJsVersion"; // 本地wasm版本号
const JS_VERSION = "2022.02.11"; // wasm版本号，如有变动请修改这个值重新获取
...
(function () {
    ...
    if (js && jsVersion == JS_VERSION) {
        ...
        GM_xmlhttpRequest({
            method: "GET",
            url: "https://static.yximgs.com/udata/pkg/acfun-player/asyncPlayerPlugins.083319.js",
            responseType: "text",
            onload: function(response) {
                wasm = response.response;
                const STR = "b.safari";
                let index = wasm.indexOf(STR) + STR.length + 3;
                let js = wasm.slice(0, index) + "Joysound=e;" + wasm.slice(index);
